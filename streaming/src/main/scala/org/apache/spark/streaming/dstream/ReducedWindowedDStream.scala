...
private[streaming]
abstract class ReducedWindowedDStream[K: ClassTag, V: ClassTag](
    parent: DStream[(K, V)],
    reduceFunc: (V, V) => V,
    invReduceFunc: (V, V) => V,
  ...
  protected val reducedStream = parent.reduceByKey(reduceFunc, partitioner)
  ...
  override def compute(event: Event): Option[RDD[(K, V)]] = {
    ...
    val allRDDs = dependencies.map(_.asInstanceOf[Dependency[(K, V)]])
...
private[streaming]
class ReducedTailWindowedDStream[K: ClassTag, V: ClassTag](
    parent: DStream[(K, V)],
    reduceFunc: (V, V) => V,
    invReduceFunc: (V, V) => V,
    filterFunc: Option[((K, V)) => Boolean],
    ...
    skipLength: Int,
    partitioner: Partitioner
  ) extends ReducedWindowedDStream[K, V](
    parent, reduceFunc, invReduceFunc, filterFunc, partitioner) {
...
class ReducedTimeWindowedDStream[K: ClassTag, V: ClassTag](
  ...
  override def shouldCompute(event: Event): Boolean = {
    ...
    dependencies.last.asInstanceOf[TimeWindowDependency[(K, V)]]
